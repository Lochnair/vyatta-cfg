                       ...
                       bool init = true)
{
   ...
   */
  if (include_root && order == PRE_ORDER && !_trv_tag_node(root)) {
  ...
  if (include_root && order == POST_ORDER && !_trv_tag_node(root)) {
    nodelist.push_back(root);
  }
}
...
_commit_exec_prio_subtree(Cstore& cs, PrioNode *proot)
{
  ...
  if (cfg) {
    ...
    if (clist.size()) {
      CstoreCPathListT cstorelist;
      for (size_t i = 0; i < clist.size(); i++) {
        tr1::shared_ptr<Cpath> cpath(clist[i].second);
        cstorelist.push_back(CstoreCPathT(
          clist[i].first == COMMIT_STATE_DELETED, cpath));
      }
      if (!cs.markCfgPathCommitted(cstorelist)) {
...
static void
_execute_hooks(CommitHook hook, Cstore& cs)
{
  const char* dir = getCommitHookDir(hook);
  if (!cs.isEmptyDir(dir)) {
...
void
CommitData::setCommitMultiValues(const vector<string>& values,
                                 const vector<CommitState>& states)
{
...
void
CommitData::setCommitValue(const string& val1, const string& val2,
                           bool def1, bool def2)
{
                              ...
                              bool in_active, bool in_working)
{
  ...
  if (def->isTagNode()) {
    ...
    cs._cfgPathGetChildNodes(pcomps, tvals, in_active);
    Cpath vpath(pcomps);
...
class SetCommitSession {
public:
    SetCommitSession() {
        char pbuf[24];
        char *sid;
        if (!(sid = getenv("UBNT_CFGD_PROC_REQ_SID"))) {
            snprintf(pbuf, 24, "%u", getppid());
            sid = pbuf;
        }
        setenv("COMMIT_SESSION_ID", sid, 1);
    }

    ~SetCommitSession() {
        unsetenv("COMMIT_SESSION_ID");
    }
};
...
{
  SetCommitSession scs;

  system("sudo /bin/rm -f /var/run/.defconfig");
   ...
   * of commit locking.
   */
  tr1::shared_ptr<Cstore::CommitLock> lock = cs.getCommitLock();
  if (!lock->locked()) {
  ...
  if (s > 0) {
    ...
    int sessions;
    bool res = cs.getNumberSession(sessions);
    if (!res || sessions > 1) {
