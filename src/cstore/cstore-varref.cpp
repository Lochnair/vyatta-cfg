                            ...
                            vtw_type_e def_type)
{
  ...
  if (cr_comp == "@") {
    ...
    if (def->isTag()) {
      ...
      _cstore->_cfgPathGetChildNodes(pcomps, cnodes, _active);
  ...
  if (handle_leaf) {
    if (def->isMulti()) {
      // multi-value node
      vector<string> vals;
      if (!_cstore->_cfgPathGetValues(pcomps, vals, _active)) {
        return;
      }
      string val;
      ...
      if (!_cstore->_cfgPathGetValue(pcomps, val, _active)) {
...
Cstore::VarRef::getValue(string& value, vtw_type_e& def_type)
{
  ...
  for (size_t i = 0; i < _paths.size(); i++) {
    ...
    if (_paths[i].second == ERROR_TYPE
        && !_cstore->_cfgPathExists(_paths[i].first, _active)) {
