...
typedef std::pair<bool, std::tr1::shared_ptr<Cpath> > CstoreCPathT;
typedef std::vector<CstoreCPathT> CstoreCPathListT;
...
public:
  ...
  /******
   * External interface for actual CLI operations:
   *   set
   *   delete
   *   activate
   *   deactivate
  ...
  bool validateCloneArgs(const Cpath& args);
  bool cloneCfgPath(const Cpath& args);
  // edit-related
  ...
  virtual bool markCfgPathCommitted(const CstoreCPathListT& clist);
  virtual bool clearCommittedMarkers() = 0;
  virtual bool commitConfig(commit::PrioNode& pnode) = 0;
  virtual bool isEmptyDir(const char *const dir) = 0;
  virtual bool getNumberSession(int& sessions) = 0;
  ...
  virtual tr1::shared_ptr<CommitLock> getCommitLock() = 0;
  ...
  virtual void enableCacheMode() {};
  virtual void disableCacheMode() {};
  ...
  // implementation functions. Those functions expect that session exists.
  // Should not be called from CLI directly.
  // set
  bool _validateSetPath(const Cpath& path_comps);
  // delete
  bool _deleteCfgPath(const Cpath& path_comps);
  // activate (actually "unmark deactivated" since it is 2-state, not 3)
  bool _validateActivatePath(const Cpath& path_comps);
  bool _unmarkCfgPathDeactivated(const Cpath& path_comps);
  // deactivate
  bool _validateDeactivatePath(const Cpath& path_comps);
  bool _markCfgPathDeactivated(const Cpath& path_comps);
  // rename
  bool _renameCfgPath(const Cpath& args);
  // copy
  bool _copyCfgPath(const Cpath& args);
  // comment
  bool _commentCfgPath(const Cpath& args);
  // discard
  bool _discardChanges();
  // move
  bool _validateMoveArgs(const Cpath& args);
  bool _moveCfgPath(const Cpath& args);
  bool _validateCloneArgs(const Cpath& args);
  bool _cloneCfgPath(const Cpath& args);
  // edit-related
  bool _getEditEnv(const Cpath& path_comps, string& env);
  bool _getEditUpEnv(string& env);
  bool _getEditResetEnv(string& env);
  // completion-related
  bool _getCompletionEnv(const Cpath& comps, string& env);
  // commit
  bool _unmarkCfgPathChanged(const Cpath& path_comps);
  // working or active config
  bool _cfgPathDeactivated(const Cpath& path_comps,
                           const bool active_cfg = false);
  bool _cfgPathLeafDeactivated(const Cpath& path_comps,
                               const bool active_cfg = false) {
    return _cfgPathMarkedDeactivated(path_comps, active_cfg);
  }
  bool _cfgPathLeafDeactivated(const vector<string>& cmarkers) {
    return marked_deactivated(cmarkers);
  }
  bool _cfgPathMarkedDeactivated(const Cpath& path_comps,
                                 bool active_cfg = false);
  bool _cfgPathExistsDA(const Cpath& path_comps, bool active_cfg = false,
                        bool include_deactivated = true) {
    return cfg_path_exists(path_comps, active_cfg, include_deactivated);
  }
  void _cfgPathGetChildNodesDA(const Cpath& path_comps, vector<string>& cnodes,
                               const bool active_cfg = false,
                               const bool include_deactivated = true);
  void _cfgPathGetChildItems(const Cpath& path_comps,
                             vector<string>& cnodes,
                             vector<string>& cmarkers,
                             const bool active_cfg = false);
  void _cfgPathChangedItems(const Cpath& path_comps,
                            vector<string>& mnodes,
                            vector<string>& dnodes,
                            bool& content_changed,
                            bool& content_opaque);
  bool _cfgPathGetValueDA(const Cpath& path_comps, string& value,
                          const bool active_cfg = false,
                          const bool include_deactivated = true,
                          const bool path_exists = false);
  bool _cfgPathGetValuesDA(const Cpath& path_comps, vector<string>& values,
                           const bool active_cfg = false,
                           const bool include_deactivated = true,
                           const bool path_exists = false);
  bool _cfgPathValueExists(const vector<string>& cmarkers) {
    return val_exists(cmarkers);
  }
  // observers for "working config" (by default) OR "active config"
  bool _cfgPathExists(const Cpath& path_comps, bool active_cfg = false) {
    return cfg_path_exists(path_comps, active_cfg, false);
  }
  bool _cfgNodeExists(const Cpath& path_comps, const bool active_cfg = false) {
    return cfg_path_exists(path_comps, active_cfg, true, 0);
  }
  void _cfgPathGetChildNodes(const Cpath& path_comps, vector<string>& cnodes,
                            const bool active_cfg = false) {
    _cfgPathGetChildNodesDA(path_comps, cnodes, active_cfg, false);
  }
  bool _cfgPathGetValue(const Cpath& path_comps, string& value,
                        bool active_cfg = false) {
    return _cfgPathGetValueDA(path_comps, value, active_cfg, false);
  }
  bool _cfgPathGetValues(const Cpath& path_comps, vector<string>& values,
                         bool active_cfg = false) {
    return _cfgPathGetValuesDA(path_comps, values, active_cfg, false);
  }
  bool _cfgPathGetComment(const Cpath& path_comps, string& comment,
                          bool active_cfg = false);
  bool _cfgPathCommentExists(const vector<string>& cmarkers) {
    return comment_exists(cmarkers);
  }
  bool _cfgPathDefault(const Cpath& path_comps, bool active_cfg = false);
  bool _cfgPathDefault(const vector<string>& cmarkers) {
    return marked_display_default(cmarkers);
  }
  /* observers for working AND active configs (at the same time).
   * MUST ONLY be used during config session.
   */
  bool _cfgPathDeleted(const Cpath& path_comps) {
    return (cfg_path_exists(path_comps, true, false)
            && !cfg_path_exists(path_comps, false, false));

  }
  bool _cfgPathAdded(const Cpath& path_comps) {
    return (!cfg_path_exists(path_comps, true, false)
            && cfg_path_exists(path_comps, false, false));
  }
  bool _cfgPathChanged(const Cpath& path_comps);
  void _cfgPathGetDeletedChildNodes(const Cpath& path_comps,
                                    vector<string>& cnodes) {
      _cfgPathGetDeletedChildNodesDA(path_comps, cnodes, false);
  }
  void _cfgPathGetDeletedValues(const Cpath& path_comps,
                                vector<string>& dvals) {
    _cfgPathGetDeletedValuesDA(path_comps, dvals, false);
  }
  // working AND active configs
  void _cfgPathGetDeletedChildNodesDA(const Cpath& path_comps,
                                      vector<string>& cnodes,
                                      bool include_deactivated = true);
  void _cfgPathGetDeletedValuesDA(const Cpath& path_comps,
                                  vector<string>& dvals,
                                  bool include_deactivated = true);
  /* observers for "effective config". can be used both during a config
   * session and outside a config session. more detailed information
   * can be found in the source file.
   */
  bool _cfgPathEffective(const Cpath& path_comps, const bool in_session = true);
  // load
...
private:
  ...
  virtual void get_all_child_item_names_impl(vector<string>& cnodes,
                                             vector<string>& cmarkers,
                                             const bool active_cfg = false) = 0;
  virtual void get_changed_items(vector<string>& mnodes,
                                 vector<string>& dnodes,
                                 bool& content_changed,
                                 bool& content_opaque) = 0;
  virtual void get_all_tmpl_child_node_names(vector<string>& cnodes) = 0;
  virtual bool write_value_vec(const vector<string>& vvec,
                               bool active_cfg = false) = 0;
  virtual bool update_value_vec() = 0;
  ...
  virtual bool unmark_display_default(bool& exists) = 0;
  virtual bool mark_deactivated() = 0;
  virtual bool unmark_deactivated() = 0;
  virtual bool unmark_deactivated_descendants() = 0;
  virtual bool mark_changed(const bool node_exists = false) = 0;
  virtual bool mark_changed_with_ancestors(const bool node_exists = false) = 0;
  ...
  virtual bool val_exists(const vector<string>& cmarkers) = 0;
  virtual bool cfg_node_exists(bool active_cfg) = 0;
  virtual bool marked_deactivated(bool active_cfg) = 0;
  virtual bool marked_deactivated(const vector<string>& cmarkers) = 0;
  virtual bool get_comment(string& comment, bool active_cfg) = 0;
  virtual bool comment_exists(const vector<string>& cmarkers) = 0;
  virtual bool marked_display_default(bool active_cfg) = 0;
  virtual bool marked_display_default(const vector<string>& cmarkers) = 0;
  ...
  bool conv_move_args_for_rename(const Cpath& args, Cpath& edit_path_comps,
                                 Cpath& rn_args);
  bool cfg_path_exists(const Cpath& path_comps, const bool active_cfg,
  ...
  void get_all_child_item_names(vector<string>& cnodes,
                                vector<string>& cmarkers,
