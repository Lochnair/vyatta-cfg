...
sub validate_iptables4_net {
  $_ = shift;
  if (!/^(\d+)\.(\d+)\.(\d+)\.(\d+)\/(\d+)$/) {
    return 0 if (!/^(\d+)\.(\d+)\.(\d+)\.(\d+)\/(\d+)\.(\d+)\.(\d+)\.(\d+)$/);
    return 0 if ($1 > 255 || $2 > 255 || $3 > 255 || $4 > 255 || $5 > 255 || $6 > 255 || $7 > 255 || $8 > 255);
    return 1;
  }
  return 0 if ($1 > 255 || $2 > 255 || $3 > 255 || $4 > 255 || $5 > 32);
  return 1;
...
sub validate_iptables4_net_negate {
  my $value = shift;
  if ($value =~ m/^\!(.*)$/) {
    $value = $1;
  }
  return validate_iptables4_net($value);
}
